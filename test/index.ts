...
import {PrivateKey, Client, utils, Signature} from '@taiyinet/ctaiyi'
...
const ctaiyiVerify: VerifyMessage = async (message: Buffer, signatures: string[], account: string) => {
    const opts = {
        hash: message,
        signatures,
...
async function createTestnetAccount(): Promise<{username: string, password: string}> {
    ...
    const response = await fetch('https://testnet.taiyi.vc/create', {
        method: 'POST',
...
describe('rpc auth', function() {
    ...
    it('handles invalid signatures', async function() {
        ...
        // valid
        await validate(signed, ctaiyiVerify)

        // invalid method
        invalid = utils.copy(signed)
        invalid.method = 'foo.bar2'
        error = await assertThrows(async () => {
            await validate(invalid, ctaiyiVerify)
        })
